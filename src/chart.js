...
const Chart = (props) => {
  ...
  const filter_communities = communities
    .filter((obj) =>
      obj.name.toLowerCase().includes(props.keyword.toString().toLowerCase())
    )
    .map((obj) => ({ name: obj.name, value: obj.value, visualMap: false }));
